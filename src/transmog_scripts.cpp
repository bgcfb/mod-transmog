...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (sT->GetEnableTransmogInfo())
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t幻化说明", EQUIPMENT_SLOT_END + 9, 0);
...
#ifdef PRESETS
        if (sT->GetEnableSets())
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/RAIDFRAME/UI-RAIDFRAME-MAINASSIST:30:30:-18:0|t管理幻化套装", EQUIPMENT_SLOT_END + 4, 0);
#endif
        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t移除所有幻化", EQUIPMENT_SLOT_END + 2, 0, "确定要移除所有幻化吗?", 0, false);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        switch (sender)
        {
    ...
    #ifdef PRESETS
            case EQUIPMENT_SLOT_END + 4: // 套装功能 菜单
            {
                if (!sT->GetEnableSets())
                {
                    OnGossipHello(player, creature);
                ...
                if (sT->GetEnableSetInfo())
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t幻化套装功能说明", EQUIPMENT_SLOT_END + 10, 0);
                ...
                if (sT->presetByName[player->GetGUID()].size() < sT->GetMaxSets())
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|t保存幻化套装", EQUIPMENT_SLOT_END + 8, 0);
            ...
            case EQUIPMENT_SLOT_END + 6: // view preset
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tUse this set", EQUIPMENT_SLOT_END + 5, action, "幻化将会绑定此物品并且无法再次交易.\n你确定要继续吗?\n\n" + sT->presetByName[player->GetGUID()][action], 0, false);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|t删除幻化套装", EQUIPMENT_SLOT_END + 7, action, "你确定要删除 " + sT->presetByName[player->GetGUID()][action] + "?", 0, false);
            ...
            case EQUIPMENT_SLOT_END + 8: // Save preset
            {
                ...
                if (canSave)
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|t保存幻化套装", 0, 0, "输入套装名称", cost*sT->GetSetCostModifier() + sT->GetSetCopperCost(), true);
    ...
    void ShowTransmogItems(Player* player, Creature* creature, uint8 slot, uint16 gossipPageNumber) // Only checks bags while can use an item from anywhere in inventory
    {
        ...
        if (oldItem)
        {
            ...
            if (sT->GetUseCollectionSystem())
            {
                ...
                if (sT->collectionCache.find(accountId) != sT->collectionCache.end())
                {
                    ...
                    if (sT->GetAllowHiddenTransmog())
                    {
                        ...
                        else
                        {
                            ...
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Hide Slot", slot, UINT_MAX, "你正在隐藏此部位.\n你确定要继续吗?\n\n" + lineEnd, 0, false);
                        }
                    }
                    ...
                    for (uint32 i = startValue; i <= endValue; i++)
                    {
                        ...
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetEntry(), "幻化将会绑定此物品并且无法再次交易.\n你确定要继续吗?\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                    }
                }
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tRemove transmogrification", EQUIPMENT_SLOT_END + 3, slot, "移除此位置的幻化?", 0, false);
            ...
            else
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                        continue;
                    ++limit;
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetGUID().GetCounter(), "幻化将会绑定此物品并且无法再次交易.\n你确定要继续吗?\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                }
        ...
        if (sendGossip) {
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t移除幻化", EQUIPMENT_SLOT_END + 3, slot, "移除此位置的幻化?", 0, false);
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|t更新菜单", EQUIPMENT_SLOT_END, slot);
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|t返回...", EQUIPMENT_SLOT_END + 1, 0);
            SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
...
private:
    ...
    void AddToDatabase(Player* player, ItemTemplate const* itemTemplate)
    {
        ...
        {
            if (showChatMessage)
                ChatHandler(player->GetSession()).PSendSysMessage( R"(|c%s|Hitem:%u:0:0:0:0:0:0:0:0|h[%s]|h|r 已经加入你的幻化收藏.)", itemQuality.c_str(), itemId, itemName.c_str());
